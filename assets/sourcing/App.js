...
function App(props) {
    ...
    const handleIdRecruteurChanged = (ti) => {
        //setItems(ti);
        axios.get(`https://localhost:8000/api/c_vs?deposePar.id=` + ti)
            .then(
                (result) => {
    ...
    const handleAllCvRequest = (cv) => {
        axios.get(`https://localhost:8000/api/c_vs?limit=4`)
            .then((result) => {
    ...
    const handleMulti = (multi) => {
        axios.get(`https://localhost:8000/api/c_vs?candidat.ville=` + multi)
            .then((result) => {
    ...
    const handleCv = (cv) => {
        axios.get(`https://localhost:8000/api/c_vs?candidat.id=` + cv)
            .then((result) => {
    ...
    const handlMotCle = (value) => {
        if (Number.isInteger(value)) {
            axios.get(`https://localhost:8000/api/metiers?rome.id=` + value)
                .then((result) => {
                    console.log(result.data[0]);
                    setCvCandidat(result.data[0]);
            ...
            axios.get(`https://localhost:8000/api/metiers?libelle=` + value)
                .then((result) => {
                    console.log(result.data[0]);
                    setCvCandidat(result.data[0]);
